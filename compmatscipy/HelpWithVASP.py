        ...
        if (remake == True) or not os.path.exists(fjson):
            ...
            with open(self.doscar) as f:
                for line in f:
                    ...
                    if (len(values) == 5) and (values[2] == str(nedos)):
                       dos_count += 1
                       continue
                    if dos_count == 1:
    ...
    def energies_to_populations(self, element='total', orbital='all', spin='summed', fjson=False, remake=False):
        ...
        if element != 'total':
            if spin != 'summed':
                ...
                up = [dos_dict[E][element]['_'.join([orbital, 'up'])] for E in energies]
                down = [dos_dict[E][element]['_'.join([orbital, 'down'])] for E in energies]
                populations = [up[i] + down[i] for i in range(len(energies))]
        else:
            if spin != 'summed':
                populations = [dos_dict[E][element][spin] for E in energies]
            else:
                up = [dos_dict[E][element]['up'] for E in energies]
                down = [dos_dict[E][element]['down'] for E in energies]
                populations = [up[i] + down[i] for i in range(len(energies))]
...
class VASPDielectricAnalysis(object):
    ...
    def dielectric_constant(self):
        ...
        Returns:
            dielectric constant (eps_0) from LOPTICS=TRUE run (float)
        """    
        outcar = os.path.join(self.calc_dir, 'OUTCAR')
        with open(outcar) as f:
            count = 0
